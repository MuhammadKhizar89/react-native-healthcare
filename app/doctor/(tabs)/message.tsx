...
import { getDoctorAppointments } from '../../../api/firebase/appointments';
        ...
        try {
          ...
          appointments.forEach(app => {
            if (!uniquePatients.has(app.patientId)) {
              uniquePatients.set(app.patientId, { id: app.patientId, name: app.patientName });
            }
          });
          if (isActive) setContacts(Array.from(uniquePatients.values()));
        } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <Text style={{ textAlign: 'center', marginTop: 20, color: '#6B7280' }}>No scheduled patients to chat with yet.</Text>
      ) : (
        <FlatList
          data={displayList}
